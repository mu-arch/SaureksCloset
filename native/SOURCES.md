...
The Lua functions are SaureksClosetSetAppearance(race,sex,skin,face,hairStyle,hairColor,facial), SaureksClosetClearAppearance(), SaureksClosetRealBody(), SaureksClosetRendererVersion(), and SaureksClosetInspect(). Renderer version is 30400; see WEAPONRY.md for the additional weapon placement API. Setter/clear return 1 on accepted request, -1 when no player is available, -2 for invalid input, -3 for unsupported native display, -4 for reentry. Accepted requests can still await model/texture loading.
