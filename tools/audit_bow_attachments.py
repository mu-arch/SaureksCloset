"""Verify center-back anchors from the installed 1.12.1 models; no model writes.
...
fixture='// Extracted from all 16 installed build-5875 character models.\n// Regenerate with tools/audit_bow_attachments.py. Test data only.\nstruct BowAttachmentFixture { unsigned race,sex,index,bone; std::array<float,3> position; };\nstatic const BowAttachmentFixture bowFixtures[]={\n'
for row in rows:
    point=row['points'][28]
    fixture+='{%d,%d,%d,%d,{{%s}}},\n'%(row['race'],row['sex']=='Female',point['index'],point['bone'],','.join('%.9ff'%v for v in point['position']))
fixture+='};\n'
path=root/'tests/bow_attachment_fixtures.h'
...
print('PASS: center-back attachment IDs, indices, bones and coordinates verified against all 16 installed models.')
