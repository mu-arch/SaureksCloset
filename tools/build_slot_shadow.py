...
def separate_shadow(overlay):
    ...
    contact = alpha.filter(ImageFilter.MaxFilter(3)).filter(ImageFilter.GaussianBlur(.9))
    ambient = alpha.filter(ImageFilter.MaxFilter(3)).filter(ImageFilter.GaussianBlur(2.5))
