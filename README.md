...
3. Open the addon's **[Installation instructions](addon/SaureksCloset/Installation%20instructions/)** folder. Copy the included `SaureksCloset.dll` into your main game folder, beside `WoW.exe`. The short [setup guide](addon/SaureksCloset/Installation%20instructions/READ%20ME.txt) explains where to drag it.
...
The addon lives in `addon/SaureksCloset/`. DLL source is in `native/`, with build tools in `tools/` and tests in `tests/`. See [the build guide](BUILDING.md) and the included license notices. Catalog attribution is documented in [CATALOG-COPYRIGHT.md](addon/SaureksCloset/CATALOG-COPYRIGHT.md) and [CATALOG-LICENSE.md](addon/SaureksCloset/CATALOG-LICENSE.md); artwork provenance is recorded in [ARTWORK.json](addon/SaureksCloset/ARTWORK.json).
